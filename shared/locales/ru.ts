export default {
  ...
  product: {
    meta: {
      title: "FRKN Box — Умный VPN для вашего дома",
      description:
        "Безопасный и надежный VPN-роутер для защиты всех устройств в вашей сети",
    },
    hero: {
      line1: "Безопасный",
      line2: "Wi-Fi с VPN",
    ...
    trust: {
      description:
        "Умный роутер на базе Raspberry, который обеспечивает анонимный и безопасный интернет, а в свободное время зарабатывает для вас деньги.",
    },
    features: {
      cpu: "1.5 ГГц",
      cpu_description: "ARM v8 процессор с частотой 1.5 ГГц",
      ram: "4 GB",
      ram_description: "LPDDR4 RAM",
      ethernet: "1 Gbit",
      ethernet_description: "Ethernet для стабильного соединения",
      wifi_support: "Поддержка Wi-Fi",
      wifi_frequency: "2.4 / 5 ГГц",
      aluminum_heatsink: "Алюминиевый радиатор",
      heatsink_description: "для бесшумного охлаждения",
      optimized_software: "Оптимизированное ПО",
      software_description: "с автоматическим обновлением",
    },
    specs: {
      title: "Аппаратное Шифрование",
      module:
        "FRKN VPN Node оснащён модулем TPM 2.0 (Trusted Platform Module), обеспечивающим глубокое крипто-шифрование на аппаратном уровне.",
      description:
        "Решение обеспечивает защиту ключей и данных от взлома, используя аппаратное хранилище для шифрования VPN-трафика. Даже в случае компрометации устройства система остается устойчивой к атакам, сохраняя безопасность данных.",
    },
    extensible: {
      title: "Доход от раздачи интернета",
      description:
        "FRKN VPN обеспечивает полную защиту трафика без логов и слежки, а также помогает обходить блокировки в цензурированных регионах. Кроме безопасности, устройство позволяет зарабатывать: вы сможете получать $10+ в месяц и токены за раздачу интернета, а ранние пользователи имеют шанс на airdrop.",
      settings:
        "Настройка занимает всего 5 минут — просто подключите питание и интернет, а всё остальное уже настроено.",
      cta: "Купить",
    },
    get: {
      headerTop: "Участвуйте в Node Sale",
      headerBottom: "получите льготные условия",
      description:
        "Наш токен запустится в мае 2025, и ранние участники смогут получить AirDrop. Рассылка нод стартует летом 2025, чтобы все владельцы смогли сразу подключиться к сети и начать зарабатывать.",
    },
  },
} as const
